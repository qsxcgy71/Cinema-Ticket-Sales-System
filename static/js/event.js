...
function populateMovieCards(movies) {
  ...
  movies.forEach(movie => {
      ...
      const normalizedMovieName = movie.name.replace(/\s+/g, '_').toLowerCase();

      // 定义支持的图片格式
      const imageFormats = ['webp', 'jpg', 'png'];
      let posterPath = `assets/${normalizedMovieName}.webp`; // 默认尝试使用 .webp 格式

      // 检查文件是否存在
      imageFormats.forEach(format => {
          const testPath = `${normalizedMovieName}.${format}`;
          const img = new Image();
          img.onload = function () {
              posterPath = testPath; // 文件存在时更新路径
          };
          img.onerror = function () {
              console.warn(`Image not found: ${testPath}`);
          };
          img.src = testPath; // 触发加载以验证文件存在
      });
      ...
      card.innerHTML = `
          ...
          <div class="card-body" style="line-height: 1.5;">
              ...
              <a class="btn btn-success buy-ticket-btn d-block mx-auto"
                 ...
                 data-time="${movie.date} ${movie.time}"
